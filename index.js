/*
]=====> MANIK JB <=====[ ]=====> YT ARYA MANIK <=====[ ]=====> 085737134572 <=====[
*/
const {
   WAConnection,
   MessageType,
...
// JAN NGEYEL AJG
            + 'ORG:OWNER BOTZ;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6285737134572:+62 857-3713-4572\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []   
limitawal = 30
...
*/
const ownerNumber = ["6285737134572@s.whatsapp.net","6285737134572@s.whatsapp.net"]
const botName = 'MANIK BOT'
const ownerName = 'MANIK'
const BarBarKey = 'IDxO1TFYnKADlX4pxcHa' // APIKEY DOWNLOAD FILE
const VhtearKey = 'BSHDBDUENDUDBNSBD737' // YANG DI YOUTUBE
...
console.log(banner.string)
   ...
   qrcode.generate(qr, { small: true })
	console.log(color('[','white'), color('!','red'), color(']','white'), color(' SUBSCRIBE YT MANIK CHANNEL'))
})

	client.on('credentials-updated', () => {
		fs.writeFileSync('./MANIK.json', JSON.stringify(client.base64EncodedAuthInfo(), null, '\t'))
		info('2', 'ingfokan cuyy...')
	})
	fs.existsSync('./MANIK.json') && client.loadAuthInfo('./MANIK.json')
	client.on('connecting', () => {
		start('2', 'MANIK Connecting...')
	})
	client.on('open', () => {
		success('2', 'MANIK Connected')
	})
...
*/
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis1 MANIK baik hati`)
				MANIK = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
				buff = await getBuffer(`https://api.xteam.xyz/magernulis2?text=${MANIK}&APIKEY=${XteamKey}`)
				...
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis2 MANIK baik hati`)
				laysha = body.slice(8)
				reply('「❗」WAIT BRO GUE NULIS DUMLU YAKAN')
				...
				client.sendMessage(from, ''+tes+'\n\n_By : ⸸MANIK⸸Panutanque._', text, { quoted: mek })
				await limitAdd(sender)
				break
// NINJALOGO				
				case 'ninjalogo':
				...
				var nin = gh.split("&")[0];
				var ja = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}ninjalogo MANIK & Gans`)
				reply(ind.wait())
...
// PORNHUB
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}pornhub MANIK & Hub`)
				reply(ind.wait())
...
//GEMBOK TEXT
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gem = gh.split("&")[0];
					var bok = gh.split("&")[1];
					if (args.length < 1) return reply(`[❗] Contoh : ${prefix}gemboktext 11 01 2021 & MANIK dan Nadia`)
					reply(ind.wait())
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					var gli = gh.split("&")[0];
					var tch = gh.split("&")[1];
					if (args.length < 1) return reply(`[❗] Contoh : ${prefix}glitchtext MANIK & Gans`)
					reply(ind.wait())
                ...
                var gh = body.slice(12)
					...
					var wm = gh.split("&")[1];
					const pref = `yang mau dijadiin quote apaan, titit?\n\ncontoh : ${prefix}bikinquote aku bukan boneka & Kata MANIK`
...
*/
		   ...
		   await limitAdd(sender)
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					truteh = await getBuffer(`https://i.ibb.co/mXtzcv9/1613024083-picsay.jpg`)
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))                
					...
					tod = await getBuffer(`https://i.ibb.co/mXtzcv9/1613024083-picsay.jpg`)
...
*/		 
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))				
					...
					try {
						...
						console.log('Error :', e)
						reply('Anjim yang mau di add di private, dahlah gosah :)')
					}
					break
			     	case 'kick':
                  if (isBanned) return reply(ind.baned())
...
*/				
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`maaf ${pushname} event mining tidak di aktifkan sama owner MANIK`)
                      if (isOwner) {
                      const one = 999999999
                      addLevelingXp(sender, one)
                      addLevelingLevel(sender, 99)
                      reply(`karena MANIK baik Bot memberikan ${one}Xp >_<`)
                      }else{
            ...
            case 'creator':
                  ...
                  client.sendMessage(from, 'Tuh Nomor Pacarku >_<, Ehh Ownerku mksdnya:v btw dia cakep:v',MessageType.text, { quoted: mek} )
					break					
/*
]=====> MAKER MENU <=====[
*/
...
har = body.slice(12)
reply('「❗」Harta Tahta Manik dong :v')
...
if (!isRegistered) return reply(ind.noregis())
if (isLimit(sender)) return reply(ind.limitend(pusname))
if (args.length < 1) return reply(`「❗」Contoh : ${prefix}cloudtext MANIK`)
cloud = body.slice(11)
